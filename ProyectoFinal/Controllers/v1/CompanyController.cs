...
public class CompanyController : ControllerBase
{
    ...
    public async Task<IActionResult> CreateCompany(string Description, string officeAddress, string telephoneNumber, string foundingDate, string nit, string nameCEO)
    {
        var roleId = User.Claims.FirstOrDefault(x => x.Type == "idRol");
        int idRol = int.Parse(roleId.Value.ToString());

        var response = await _companyBLL.CreateCompany(Description,officeAddress,telephoneNumber,foundingDate,nit,nameCEO, idRol);
        if (!response.IsSuccess)
            return BadRequest(response);
        return Ok(response);
    }
